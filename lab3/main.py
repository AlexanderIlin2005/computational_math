...
def fix_layout(text):
    ...
    fypad = "йцукенгшщзфывапролдячсмить"

    # Создаём таблицу перевода из русской раскладки в английскую
    ru_to_en = str.maketrans(fypad + fypad.upper(), qwerty + qwerty.upper())
...
if __name__ == "__main__":
    while (True):
        print("Выберите номер функции или exit для завершения):")
        print("1. x^2")
        print("2. sin(x)")
        print("3. e^x")
        print("4. 1/x^2")
        ...
        choosen_f = input("Ваш выбор: ").replace(' ', '')

        if fix_layout(choosen_f.lower()) == "exit":
            break

        if not choosen_f.isdigit():
            print("Номер функции должен быть числом. Попробуйте снова")
            continue

        choosen_f = int(choosen_f) - 1

        if (choosen_f not in [0, 1, 2, 3, 4, 5, 6, 7, 8]):
            print("Пожалуйста, выберите корректный номер функции(строго от 1 до 9)!\n")
            continue
        ...
        if len(breakpoints) != 0:
            print(f"! Обнаружены точка разрыва: функция имеет разрыв или не существует в точках {breakpoints}.")
            ...
            epsilon = float(input("Введите требуемую точность вычислений (по умолчанию 0.1): ") or 0.1)
